...
//if (typeof data.checkNude == "undefined" || data.checkNude == false) return;
  ...
  const ress = await axios.get(`https://api.anhyeuem1505.repl.co/imgur?link=${encodeURIComponent(dataa.url)}`)
	const link = ress.data.uploaded.image
  var res = await axios.get(`https://manhict.tech/api/checknude?url=${link}&apikey=PSbdGqnr`);
  var img = res.data.result.data
  if(img > 80) { return api.sendMessage(`Phát hiện ảnh nude trong box của bạn\nMức độ: ${res.data.result.NSFW_Prob}`, event.threadID, event.messageID) 
  }
  }
}
...
const ress = await axios.get(`https://api.anhyeuem1505.repl.co/imgur?link=${encodeURIComponent(event.messageReply.attachments[0].url)}`)
	const link = ress.data.uploaded.image
  var res = await axios.get(`https://manhict.tech/api/checknude?url=${link}&apikey=PSbdGqnr`);
  var img = res.data.result.data
  return api.sendMessage(`Mức độ 18+: ${res.data.result.NSFW_Prob}`, event.threadID, event.messageID) 
}
else {
