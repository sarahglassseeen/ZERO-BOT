        ...
        switch (logMessageType) {
            case "log:thread-admins": {
                 if (logMessageData.ADMIN_EVENT == "add_admin") {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`» [ CẬP NHẬT NHÓM ]\n» Đã cập nhật ${logMessageData.TARGET_ID} Trở Về Thành Viên`, threadID, async (error, info) => {
                        if (global.configModule[this.config.name].autoUnsend) {
                            await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                            return api.unsendMessage(info.messageID);
                        } else return;
            ...
            case "log:user-nickname": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage(`» [ CẬP NHẬT NHÓM ]\n» Đã cập nhật biệt danh của ${logMessageData.participant_id} Thành: ${(logMessageData.nickname.length == 0) ? "Tên Cũ": logMessageData.nickname}`, threadID, async (error, info) => {
                    if (global.configModule[this.config.name].autoUnsend) {
                        await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                        return api.unsendMessage(info.messageID);
                    } else return;
            ...
            case "log:thread-name": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage(`» [ CẬP NHẬT NHÓM ]\n» Đã cập nhật tên box thành ${dataThread.threadName}`, threadID, async (error, info) => {
                    if (global.configModule[this.config.name].autoUnsend) {
                        await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                        return api.unsendMessage(info.messageID);
                    } else return;
